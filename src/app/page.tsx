...
import { MessageSquarePlus, Loader2, MapPin } from 'lucide-react';
import { ControlPosition, useMapsLibrary } from '@vis.gl/react-google-maps';
...
const CONTENT_EFFECTIVE_MAX_HEIGHT = `calc(100vh - ${HEADER_HEIGHT} - ${PAGE_VERTICAL_PADDING} - 12rem)`; 

type PickingLocationType = 'origin' | 'destination' | null;
...
export default function HomePage() {
  ...
  const [pickingLocationFor, setPickingLocationFor] = useState<PickingLocationType>(null);
  ...
  const handlePickLocation = (type: 'origin' | 'destination') => {
    ...
    toast({ title: "Seleccionar en Mapa", description: `Click en el mapa para seleccionar ${type === 'origin' ? 'el origen' : 'el destino'}.`});
  };

  const handleMapClick = async (coords: GeoCoordinates) => {
    if (pickingLocationFor && geocoder) {
      try {
        ...
        if (response.results[0]) {
          ...
          toast({ title: "Ubicación Seleccionada", description: `${pickingLocationFor === 'origin' ? 'Origen' : 'Destino'} establecido: ${address}` });
        } else {
          toast({ variant: "destructive", title: "Error", description: "No se pudo encontrar una dirección para este punto."});
        }
      } catch (error) {
        console.error("Geocoding error:", error);
        toast({ variant: "destructive", title: "Error de Geocodificación", description: "No se pudo convertir las coordenadas a dirección." });
      } finally {
            ...
            <div className="flex-grow h-[50vh] lg:h-full relative">
              <Suspense fallback={
                ...
                <MapComponent
                  ...
                  onMapClick={pickingLocationFor ? handleMapClick : undefined}
              ...
              {pickingLocationFor && (
                <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground p-2 rounded shadow-lg text-sm z-10 pointer-events-none">
                  ...
                  Click en el mapa para seleccionar {pickingLocationFor === 'origin' ? 'el ORIGEN' : 'el DESTINO'}.
                </div>
              )}
            </div>
          </section>
